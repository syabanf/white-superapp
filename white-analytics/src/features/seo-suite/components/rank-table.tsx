...
function DeltaCell({ change }: { change: RankKeywordRow["change"] }) {
  ...
  if (change.kind === "lost")
    return <span className="text-xs font-medium text-negative">{s.declinedKw}</span>;
  ...
  return (
    <span className={`tabular text-xs font-medium ${up ? "text-positive" : "text-negative"}`}>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span
          ...
          title={row.original.device === "MOBILE" ? t.seo.mobile : t.seo.desktop}
        >
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.position == null ? (
          ...
          <span className="tabular text-base font-semibold">{formatNumber(row.original.position)}</span>
        ),
    },
    {
      id: "delta",
      header: s.deltaCol,
      meta: right,
      accessorFn: (r) => r.change.delta ?? -999,
      cell: ({ row }) => <DeltaCell change={row.original.change} />,
    },
    {
      accessorKey: "best",
      header: s.bestCol,
      meta: right,
      cell: ({ row }) =>
        row.original.best == null ? "–" : <span className="tabular">{formatNumber(row.original.best)}</span>,
    },
    {
      accessorKey: "url",
      header: s.urlCol,
    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.volume == null ? (
          ...
          <span className="tabular">{formatNumber(row.original.volume)}</span>
        ),
    },
    {
      ...
      header: s.kd,
      meta: right,
      cell: ({ row }) => <DifficultyBadge kd={row.original.difficulty} />,
    },
    {
      id: "serp",
      header: s.serpFeatures,
      enableSorting: false,
      cell: ({ row }) => <SerpFeatureChips features={row.original.serpFeatures} max={2} />,
    },
    {
      id: "spark",
      header: s.history30,
      enableSorting: false,
      cell: ({ row }) => <Sparkline values={row.original.spark} accent="var(--chart-1)" />,
    },
    ...(canManage
      ? [
          {
            ...
            header: "",
            enableSorting: false,
            cell: ({ row }) => <RowMenu row={row.original} clientId={clientId} canManage={canManage} />,
          } satisfies ColumnDef<RankKeywordRow, unknown>,
        ]
      : []),
  ];
