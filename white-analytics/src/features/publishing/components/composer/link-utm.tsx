...
export function LinkUtmFields({
  ...
  const field = (key: keyof Utm, label: string) => (
    <div className="space-y-1">
      ...
      <Input id={`utm-${key}`} value={utm[key] ?? ""} onChange={(e) => onUtm({ ...utm, [key]: e.target.value })} disabled={disabled} className="h-8 font-mono text-xs" />
    </div>
  );
  return (
    <div className="space-y-3">
      <div className="space-y-1">
        ...
        <Input id="post-link" type="url" inputMode="url" value={linkUrl} onChange={(e) => onLink(e.target.value)} placeholder={p.linkPlaceholder} disabled={disabled} className="h-9" />
      </div>
      {linkUrl.trim() ? (
        <>
          ...
          <div>
            ...
            <p className="break-all rounded-md bg-muted/60 px-2.5 py-1.5 font-mono text-[11px] leading-relaxed">{finalUrl}</p>
    ...
    <div className="space-y-4">
      {showFirstComment ? (
        <div className="space-y-1">
          ...
          <Textarea id="post-first-comment" value={firstComment} onChange={(e) => onFirstComment(e.target.value)} placeholder={p.firstCommentPlaceholder} rows={2} disabled={disabled} className="text-sm" />
        </div>
      ) : null}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {labels.map((l) => (
            <Badge key={l} variant="secondary" className="gap-1 pr-1">
              {l}
              <button type="button" onClick={() => onLabels(labels.filter((x) => x !== l))} aria-label={`${p.remove} ${l}`} className="rounded-full hover:bg-foreground/10" disabled={disabled}>
                <X className="size-3" />
              </button>
            </Badge>
          ))}
