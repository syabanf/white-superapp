...
export type OverviewSyncJob = { id: string; kind: string; status: string; startedAt: Date; message: string | null; error: string | null };

/** Social summary for own (non-competitor) accounts across platforms. */
export const getOverviewSocial = cache(async (clientId: string, range: DateRange, previous: DateRange): Promise<OverviewSocial> => {
  ...
  if (accounts.length === 0) {
    ...
    return { hasData: false, followers: 0, followersDelta: zero, followersSpark: [], posts: 0, postsDelta: zero, engagementRate: 0, engagementRateDelta: zero, reach: 0, reachDelta: zero, accounts: [] };
  }
  ...
  const [snapshots, posts, prevPosts] = await Promise.all([
    ...
    db.socialPost.findMany({
      ...
      select: { socialAccountId: true, likes: true, comments: true, shares: true, saves: true, publishedAt: true },
    }),
    db.socialPost.findMany({
      where: { socialAccountId: { in: ids }, publishedAt: { gte: previous.from, lt: addDays(previous.to, 1) } },
      select: { socialAccountId: true, likes: true, comments: true, shares: true, saves: true, publishedAt: true },
    }),
  ]);
  ...
  return {
    ...
    accounts: accounts.map((a) => ({ platform: a.platform, username: a.username, followers: followersByAcc.get(a.id) ?? 0 })),
  };
});

export const getOverviewSeo = cache(async (clientId: string, range: DateRange, previous: DateRange): Promise<OverviewSeo> => {
  const props = await db.seoProperty.findMany({ where: { clientId }, select: { id: true } });
  const zero = computeDelta(0, 0);
  if (props.length === 0) {
    return { hasData: false, clicks: 0, clicksDelta: zero, clicksSpark: [], impressions: 0, impressionsDelta: zero, ctr: 0, ctrDelta: zero, position: 0, positionDelta: zero, health: null, daily: [] };
  }
  const ids = props.map((p) => p.id);
  const [rows, prevRows, latestAudit, latestCrawl] = await Promise.all([
    db.seoDailyMetric.findMany({ where: { propertyId: { in: ids }, date: { gte: range.from, lte: range.to } }, orderBy: { date: "asc" } }),
    db.seoDailyMetric.findMany({ where: { propertyId: { in: ids }, date: { gte: previous.from, lte: previous.to } } }),
    db.seoAudit.findFirst({ where: { propertyId: { in: ids }, strategy: "MOBILE" }, orderBy: { runAt: "desc" } }),
    db.seoCrawl.findFirst({ where: { propertyId: { in: ids }, status: "SUCCESS" }, orderBy: { startedAt: "desc" }, include: { _count: { select: { issues: true } }, issues: { select: { severity: true } } } }),
  ]);
  const cur = aggregateSearch(rows);
  const prev = aggregateSearch(prevRows);
  const daily = fillDaily(range, rows, (r) => r.date, ["clicks", "impressions"], { missing: null }) as { date: string; clicks: number; impressions: number }[];
...
export const getOverviewAds = cache(async (clientId: string, range: DateRange, previous: DateRange): Promise<OverviewAds> => {
  const accounts = await db.adAccount.findMany({ where: { clientId }, select: { id: true } });
  const ids = accounts.map((a) => a.id);
  const empty = aggregateAds([]);
  if (ids.length === 0) {
    const zero = computeDelta(0, 0);
    return { hasData: false, kpis: empty, prev: empty, spendDelta: zero, resultsDelta: zero, cprDelta: zero, spendSpark: [], daily: [] };
  }
  const [rows, prevRows] = await Promise.all([
    db.adDailyInsight.findMany({ where: { adAccountId: { in: ids }, adId: { not: null }, date: { gte: range.from, lte: range.to } }, orderBy: { date: "asc" } }),
    db.adDailyInsight.findMany({ where: { adAccountId: { in: ids }, adId: { not: null }, date: { gte: previous.from, lte: previous.to } } }),
  ]);
  ...
  const daily = fillDaily(range, conv(rows).length ? rows.map((r) => (NON_CONVERSION_RESULT_TYPES.has(r.resultType ?? "") ? { ...r, results: 0 } : r)) : rows, (r) => r.date, ["spend", "results"]) as { date: string; spend: number; results: number }[];
  return {
    hasData: rows.length > 0 || prevRows.length > 0,
    kpis,
    prev,
